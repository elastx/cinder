...
class TestQemuImgInfo(test.TestCase):
    ...
    @mock.patch('cinder.utils.execute')
    def test_qemu_img_info_includes_format_specific(self, mock_exec):
        """We can't be sure that we are using oslo.utils>=4.1.0"""
        qemu_img_info = '''
        {
        ...
        mock_err = mock.sentinel.err
        mock_exec.return_value = (qemu_img_info, mock_err)

        downgraded_info_obj = imageutils.QemuImgInfo(qemu_img_info,
                                                     format='json')
        if hasattr(downgraded_info_obj, 'format_specific'):
            del downgraded_info_obj.format_specific
